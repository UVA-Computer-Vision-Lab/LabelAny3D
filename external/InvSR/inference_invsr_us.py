...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        "--cfg_path", type=str, default="../external/InvSR/configs/sample-sd-turbo.yaml", help="Configuration path.",
    )
    parser.add_argument(
...
def get_configs(args):
    ...
    sd_path = args.sd_path if args.sd_path else "../external/checkpoints"  # relative to src/
    ...
    if args.started_ckpt_path:
        ...
        started_ckpt_dir = "../external/checkpoints"  # relative to src/
...
def main():
    ...
    dataset_list = {
                    'KITTI_val': 'datasets/Omni3D/KITTI_val.json',}

    root = '../dataset'
    root_out = '../dataset/enhanced'

    for dataset_name, dataset_pth in dataset_list.items():
        with open(os.path.join(root, dataset_pth), 'r') as f:
            dataset = json.load(f)

        for img in dataset["images"]:
            im_path = os.path.join(root, img['file_path'])
            out_path = os.path.join(root_out, "/".join(img['file_path'].split("/")[:-1]))
            sampler.inference(im_path, out_path=out_path, bs=args.bs)

def enhance_on_COCO(split):
    args = get_parser()
    ...
    dataset_path = "../dataset/coco/images"
    dataset_out_path = f"../dataset/coco/enhanced_images/{split}"
    ...
    for img in files:
        ...
        out_path = os.path.join(dataset_out_path, "/".join(img.split("/")[:-1]))

        sampler.inference(im_path, out_path=out_path, bs=args.bs)


def enchance_single_image(scene_dir):
    args = get_parser(description="My CLI tool")

    configs = get_configs(args)

    sampler = InvSamplerSR(configs)

    out_path = os.path.join(scene_dir, "enhanced")

    sampler.inference(f"{scene_dir}/input.png", out_path=out_path, bs=args.bs)

    return f"{out_path}/input.png"


if __name__ == '__main__':
    enhance_on_COCO('val2017')
