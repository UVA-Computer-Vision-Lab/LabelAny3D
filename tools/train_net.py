...
def setup(args):
    ...
    # very simple registration of datasets, just record the json file and image root, no record of the categories names and thing_dataset_id_to_contiguous_id
    for dataset_name in cfg.DATASETS.TRAIN:
        simple_register(dataset_name, filter_settings, folder_name=cfg.DATASETS.FOLDER_NAME, filter_empty=True, is_metric=dataset_name in cfg.DATASETS.METRIC_DATASETS)
    
    # # dataset_names_test = cfg.DATASETS.TEST_NOVEL + cfg.DATASETS.TEST_BASE # TODO: when evaluating on coco3d benchmark, this line works
    dataset_names_test = cfg.DATASETS.TEST # TODO: when evaluating on original ovmono3d benchmark, this line works    dataset_names_test = cfg.DATASETS.TEST

    for dataset_name in dataset_names_test:
        if not(dataset_name in cfg.DATASETS.TRAIN):
            simple_register(dataset_name, filter_settings, folder_name=cfg.DATASETS.FOLDER_NAME, filter_empty=False, is_metric=True) # TODO: currently all test datasets are using metric scale
    return cfg
