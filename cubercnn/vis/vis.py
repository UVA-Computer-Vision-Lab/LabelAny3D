        ...
        for instance in im_obj['instances']:
            ...
            if write_sample:
                ...
                if score > thres:
                    ...
                    draw_text(im_pred_3d, cat, [x1, y1, w, h], scale=0.75*im.shape[0]/500, bg_color=color)
        if write_sample:
...
def draw_scene_view(im, K, meshes, text=None, scale=1000, R=None, T=None, zoom_factor=1.0, mode='front_and_novel', blend_weight=0.80, blend_weight_overlay=1.0, ground_bounds=None, canvas=None, zplane=0.05, text_scale_factor=0.75):
    """
    ...
    Args:
        ...
        text_scale_factor (float): scale factor for text size, default is 0.75
    """
    if R is None:
    ...
    if mode == '2D_only':
        ...
        for mesh_idx in reversed(np.argsort([mesh.verts_padded().cpu().mean(1)[0, 1] for mesh in meshes])):
            ...
            if text is not None:
                draw_text(im_drawn_rgb, '{}'.format(text[mesh_idx]), [x1, y1], scale=text_scale_factor*im_drawn_rgb.shape[0]/500, bg_color=color)
        ...
        if mode == 'front':
            ...
            for mesh_idx in reversed(np.argsort([mesh.verts_padded().cpu().mean(1)[0, 1] for mesh in meshes_novel])):
                ...
                if text is not None:
                    draw_text(im_novel_view, '{}'.format(text[mesh_idx]), [x1, y1], scale=text_scale_factor*im_novel_view.shape[0]/500, bg_color=color)
