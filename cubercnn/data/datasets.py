...
def get_filter_settings_from_cfg(cfg=None):
    ...
    if cfg is None:
        ...
        return {
            ...
            # 'category_names_coco3d': cfg.DATASETS.CATEGORY_NAMES_COCO3D,
...
def simple_register(dataset_name, filter_settings, folder_name='Omni3D', filter_empty=False, datasets_root_path=None, is_metric=True):

    if datasets_root_path is None:
        if dataset_name.split('_')[1] == 'test' or dataset_name == 'COCO3D_val':
    ...
    DatasetCatalog.register(dataset_name, lambda: load_omni3d_json(
        ...
        dataset_name, filter_settings, filter_empty=filter_empty, is_metric=is_metric
    ))
...
def load_omni3d_json(json_file, image_root, dataset_name, filter_settings, filter_empty=False, is_metric=True):
    ...
    # if dataset_name == "COCO3D_train":
    #     cat_ids = sorted(coco_api.getCatIds(filter_settings['category_names_coco3d']))
    # else:
    ...
    if dataset_name.endswith(("_novel", "_test")) or dataset_name == "COCO3D_val":
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        # if (has_valid_annotation or (not filter_empty)) and len(objs) > 0:
