...
from detectron2.structures import Instances, Boxes, pairwise_iou, pairwise_ioa, BoxMode
...
class ROIHeads3D(StandardROIHeads):
    ...
    def forward(self, images, features, proposals, Ks, im_scales_ratio, targets=None, annos=None):
        ...
        if self.training:
            ...
            for proposal in proposals:
                proposal.pred_boxes = proposal.proposal_boxes
            if self.loss_w_3d > 0:
            ...
            # # add classes, 2D boxes
            # all_classes = torch.tensor([int(obj["category_id"]) for obj in annos[0]], dtype=torch.int64)
            # target.pred_classes = all_classes[all_classes != -1]
            # all_boxes = [BoxMode.convert(obj["bbox"], obj["bbox_mode"], BoxMode.XYXY_ABS) for obj in annos[0]]
            # target.pred_boxes = Boxes([[coord / im_scales_ratio[0] for coord in box] for box in all_boxes])[all_classes != -1]
            # target.scores = torch.ones_like(target.pred_classes).float()
    ...
    def _forward_cube(self, features, instances, Ks, im_current_dims, im_scales_ratio):
        ...
        if self.training:
            ...
            gt_is_metric = torch.cat([p.gt_is_metric for p in proposals], dim=0)
        else:
            proposals = instances
        ...
        if self.training:
            ...
            # use the relative loss
            # pred_to_gt_ratio = (gt_boxes3D[:, 2] / cube_z) 
            pred_to_gt_ratio = torch.where(
                gt_is_metric, 
                torch.ones_like(cube_z), 
                gt_boxes3D[:, 2] / cube_z
            )
            cube_z = cube_z * pred_to_gt_ratio
            cube_dims = cube_dims * pred_to_gt_ratio.unsqueeze(1)
