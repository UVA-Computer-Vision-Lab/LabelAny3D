...
class RCNN3D(GeneralizedRCNN):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        proposals = generate_augmented_proposals_from_targets(gt_instances, im_dims)
    ...
    def inference(
        ...
        if type(batched_inputs == list) and np.any(['oracle2D' in b for b in batched_inputs]):
            ...
            results, _ = self.roi_heads(images, features, oracles, Ks, im_scales_ratio, None, annos=annotations)
...
def generate_augmented_proposals_from_targets(
    targets: List[Instances],
    image_size: List[tuple],
    shift_scale_range: float = 0.05,
    size_jitter_range: float = 0.1,
    num_proposals_per_image: int = 128
) -> List[Instances]:
    """
    Generate proposals from targets with 2D bounding boxes slightly perturbed.
    Ensures each image has exactly num_proposals_per_image proposals.
    
    Args:
        targets: List[Instances], each with gt_boxes and gt_classes
        image_size: list of (height, width) tuples for each image
        shift_scale_range: max relative shift in x/y
        size_jitter_range: max relative size change (w/h)
        num_proposals_per_image: fixed number of proposals per image
        
    Returns:
        proposals: List[Instances], containing only proposal boxes and objectness scores
    """
    
    proposals = []

    for target, (H, W) in zip(targets, image_size):
        target = target[target.gt_classes >= 0]
        gt_boxes = target.gt_boxes.tensor.clone()  # (N, 4)
        device = gt_boxes.device  # Get the device from gt_boxes
        
        # Number of ground truth boxes
        num_gt = len(gt_boxes)
        
        # If we have more ground truth boxes than needed, randomly sample
        if num_gt >= num_proposals_per_image:
            # Randomly select boxes
            indices = torch.randperm(num_gt, device=device)[:num_proposals_per_image]
            gt_boxes = gt_boxes[indices]
            
            # Create proposals directly from selected ground truths
            proposals_i = Instances((H, W))
            proposals_i.proposal_boxes = Boxes(gt_boxes)
            proposals_i.objectness_logits = torch.ones(len(gt_boxes), device=device)
            
        # If we have fewer ground truth boxes than needed, generate multiple perturbations
        else:
            # How many copies of each box we need
            repeats_needed = (num_proposals_per_image + num_gt - 1) // num_gt
            
            # Repeat boxes
            repeated_boxes = gt_boxes.repeat_interleave(repeats_needed, dim=0)
            
            # Trim to exact number needed
            repeated_boxes = repeated_boxes[:num_proposals_per_image]
            
            # compute centers and sizes
            x1, y1, x2, y2 = repeated_boxes.unbind(dim=1)
            cx = (x1 + x2) / 2
            cy = (y1 + y2) / 2
            w = x2 - x1
            h = y2 - y1

            # random jitter: position shift and scale change
            shift_x = (torch.rand_like(cx) - 0.5) * 2 * shift_scale_range * w
            shift_y = (torch.rand_like(cy) - 0.5) * 2 * shift_scale_range * h
            scale_w = 1.0 + (torch.rand_like(w) - 0.5) * 2 * size_jitter_range
            scale_h = 1.0 + (torch.rand_like(h) - 0.5) * 2 * size_jitter_range
            ...
            new_x1 = (new_cx - new_w / 2).clamp(min=0, max=W)
            new_y1 = (new_cy - new_h / 2).clamp(min=0, max=H)
            new_x2 = (new_cx + new_w / 2).clamp(min=0, max=W)
            new_y2 = (new_cy + new_h / 2).clamp(min=0, max=H)

            perturbed_boxes = torch.stack([new_x1, new_y1, new_x2, new_y2], dim=1)

            proposals_i = Instances((H, W))
            proposals_i.proposal_boxes = Boxes(perturbed_boxes)
            proposals_i.objectness_logits = torch.ones(len(perturbed_boxes), device=device)
